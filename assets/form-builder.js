...
window.ExercAIse.FormBuilder = (() => {
  ...
  const buildForm = (filePath, raw, isJSON) => {
    const exercises = isJSON ? deps.extractExercisesFromJSON(raw) : deps.extractExercisesFromMarkdown(raw);
    ...
    const createExerciseCard = (title, presetRows, savedRows, headerHTML, opts) => {
      ...
      const snapshotLastRow = () => {
        ...
        for (let i = 0; i < inputs.length; i++) {
          const name = inputs[i].getAttribute('data-name');
    ...
    for (let ai = 0; ai < exNodes.length; ai++) {
      ...
      let meta0 = null; try { meta0 = metaRaw0 ? JSON.parse(metaRaw0) : null; } catch (e) { meta0 = null; }
      ...
      const card = createExerciseCard(normTitle, preset, savedRows, headerHTML, { explicitLogType: (meta0 && meta0.logType) ? meta0.logType : null });
    ...
    const inferLogTypeFromCard = (card) => {
      try {
        ...
        if (a) {
          ...
          if (raw) { const m = JSON.parse(raw); if (m && m.logType) return m.logType; }
        }
      } catch (e) {}
