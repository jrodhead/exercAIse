...
(() => {
  ...
  const parseReadmeForLatest = (contents) => {
  ...
  const renderMarkdownBasic = (md) => {
    ...
    html = html.replace(/\[(.*?)\]\((.*?)\)/g, (_, text, url) => {
      ...
      if (seg?.[1]) {
        ...
        finalUrl = finalUrl.replace(/([^:])\/+/g, (m0, p1) => p1 + '/');
      }
  ...
  const { slugify, parseHMSToSeconds, secondsToHHMMSS, extractExercisesFromMarkdown, 
          parseMarkdownPrescriptions, extractExercisesFromJSON, parseJSONPrescriptions } = window.ExercAIse.SessionParser;
    ...
    try {
      ...
      for (let i = 0; i < anchors.length; i++) {
        ...
        if (m?.[1]) {
          ...
          fixed = fixed.replace(/([^:])\/+/g, (m0, p1) => p1 + '/');
