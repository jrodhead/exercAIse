(() => {
  ...
  const status = (msg) => { if(!statusEl) return; statusEl.textContent = msg || ''; };

  const qs = () => {
    const q = {};
    const s = String(window.location.search||'').replace(/^\?/, '').split('&');
    for (let i=0;i<s.length;i++){ const kv = s[i].split('='); if (kv[0]) q[decodeURIComponent(kv[0])] = decodeURIComponent(kv[1]||''); }
    return q;
  };
  ...
  const renderExercise = (data) => {
    ...
    if (phEl){
      ...
      const pairs = [];
      const addHint = (k,label) => { if (ph?.[k]) pairs.push(`<li><span class="muted">${label}:</span> ${ph[k]}</li>`); };
      addHint('load','Load'); 
      addHint('reps','Reps'); 
      addHint('time','Time'); 
  ...
  const start = () => {
    ...
    xhrGet(path, (err, text) => {
      ...
      xhrGet(mdPath, (err2, md) => {
        ...
        const data = { name: name, equipment: [], tags: [], cues: cues.slice(0,8), safety: '', variations: [] };
        renderExercise(data);
        loadHistory(key);
      });
    });
