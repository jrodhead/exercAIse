...
- Structure the JSON into `sections` with consistent `type` values (e.g., Warm-up, Strength/Conditioning, Accessory/Core, Cooldown/Recovery). Avoid generic names like "Plan".
  - **Section displayMode (required)**: Every section must include `"displayMode": "reference" | "log"`. Use `reference` for Warm-up, Cooldown/Recovery, Recovery, Mobility, and any section where the athlete should **not** log data. Use `log` for Strength, Conditioning, Accessory/Core, Skill Work, or any main training stimulus. Always set the field explicitly; legacy inference is only for archival files. (The old per-exercise `loggable` flag has been removed.)
