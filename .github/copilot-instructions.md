...
- **performed/**: User performance logs in nested JSON format (auto-generated by app)
  - **performed/index.json**: Manifest of all performance logs with metadata
  - **Nested format**: Preserves workout organization (sections, supersets/circuits with rounds)
  - **performed/archive/**: Historical logs migrated from flat format (November 2025)
...
- **schemas/**: JSON schemas defining data contracts
  ...
  - `performance.schema.json` (nested performance log format)
...
- **Performance Tracking**: App collects data in nested format (preserving structure) for all workouts, exports to `performed/`, which informs future AI decisions
  - **Nested structure**: Preserves workout organization (sections, supersets/circuits with rounds) for superior fatigue analysis
- **Naming Conventions**: 
...
- **Data Storage**:
  ...
  - Export format: nested JSON to `performed/`
- **Testing**: 
...
**Complete Migration (November 2025)**:
...
- Round-by-round fatigue patterns now visible for all sessions

**Current Usage**:
- All workouts use nested format (JSON)
- App automatically exports in nested format
- All AI analysis uses nested structure for fatigue patterns
4. **Exercise JSONs are source of truth** - Always link to `exercises/*.json`, not markdown
5. **History informs AI decisions** - App provides clean data; AI interprets it
