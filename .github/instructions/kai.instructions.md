...
- Exercise linking: Every exercise name in a workout must be a Markdown link to its detail page in the `exercises/` directory. Keep using links like `[Goblet Squat](../exercises/goblet_squat.md)` for compatibility; the app routes these to `exercises/goblet_squat.json` when present.
